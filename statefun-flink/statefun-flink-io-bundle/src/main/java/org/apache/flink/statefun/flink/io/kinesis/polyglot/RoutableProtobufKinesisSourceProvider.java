...
import static org.apache.flink.statefun.flink.io.kinesis.polyglot.KinesisIngressSpecJsonParser.optionalAwsCredentials;
import static org.apache.flink.statefun.flink.io.kinesis.polyglot.KinesisIngressSpecJsonParser.optionalAwsRegion;
...
public final class RoutableProtobufKinesisSourceProvider implements SourceProvider {
  ...
  private static <T> KinesisIngressSpec<T> asKinesisIngressSpec(IngressSpec<T> spec) {
    ...
    optionalAwsRegion(json).ifPresent(kinesisIngressBuilder::withAwsRegion);
    optionalAwsCredentials(json).ifPresent(kinesisIngressBuilder::withAwsCredentials);
    optionalStartupPosition(json).ifPresent(kinesisIngressBuilder::withStartupPosition);
    clientConfigProperties(json)
        .entrySet()
        .forEach(
            entry ->
    ...
    Map<String, RoutingConfig> routableStreams = routableStreams(json);

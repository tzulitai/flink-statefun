...
final class JsonModule implements StatefulFunctionModule {
  ...
  private final JsonNode moduleSpecRootNode;
  private final FormatVersion formatVersion;

  public JsonModule(JsonNode moduleSpecRootNode, FormatVersion formatVersion, URL moduleUrl) {
    this.moduleSpecRootNode = Objects.requireNonNull(moduleSpecRootNode);
  ...
  public void configure(Map<String, String> conf, Binder binder) {
    try {
      ENTITIES.forEach(entity -> entity.bind(binder, moduleSpecRootNode, formatVersion));
    } catch (Throwable t) {

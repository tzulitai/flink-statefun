...
public final class PersistedRemoteFunctionValues {
  ...
  @Persisted private final Map<String, PersistedValue<byte[]>> managedStates;

  public static PersistedRemoteFunctionValues fromStateSpecs(List<StateSpec> stateSpecs) {
    final Map<String, PersistedValue<byte[]>> managedStates = new HashMap<>(stateSpecs.size());
    stateSpecs.forEach(
        spec -> {
          final String stateName = spec.name();
          managedStates.put(
              stateName,
              PersistedValue.of(
                  stateName,
                  byte[].class,
                  Expiration.expireAfterReadingOrWriting(spec.ttlDuration())));
        });
    return new PersistedRemoteFunctionValues(managedStates);
  }

  private PersistedRemoteFunctionValues(Map<String, PersistedValue<byte[]>> managedStates) {
